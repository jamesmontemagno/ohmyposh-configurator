/**
 * Configuration Exporter for MCP Server
 * Adapted from src/utils/configExporter.ts for Node.js environment
 */

import yaml from 'js-yaml';
import TOML from '@iarna/toml';
import type { OhMyPoshConfig, Block, Segment, Tooltip } from '../../src/types/ohmyposh.js';
...
export function exportToToml(config: OhMyPoshConfig): string {
  ...
  const cleanedForToml = JSON.parse(
    JSON.stringify(cleanedConfig, (_, value) => (value === null ? undefined : value))
  );
